...
function runtimes(seconds) {
  ...
  const hDisplay = h < 10 ? '0' + h + ':' : h > 0 ? h + ':' : '';
  const mDisplay = m < 10 ? '0' + m + ':' : m > 0 ? m + ':' : '';
  const sDisplay = s < 10 ? '0' + s : s > 0 ? s : '';
  return dDisplay + hDisplay + mDisplay + sDisplay;
}
...
async function cerpen(category) {
  return new Promise(async (resolve, reject) => {
    ...
    let length; const judul = title.replace(/\s/g, '-');
    try {
...
const someincludes = ( data, id ) => {
  const res = data.find((el) => id.includes(el) );
