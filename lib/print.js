...
console.log(`╭──» ${wm} 𓃠 «───✦ 
┊ ${chalk.green('%s')} ${chalk.white(chalk.white('%s'))}ㅤ${chalk.white(chalk.white('%s'))} ${chalk.white('%s [%s %sB]')}
┊ ${chalk.green('%s')} ${chalk.white('%s%s')} ${chalk.green('%s')} ${chalk.white(chalk.green('%s'))}
╰────────────✦`.trim(),
...
sender,
m ? m.exp : '?',
user ? '|' + user.messaggi + '|' + user.warn : '' + ('|' + user.messaggi),
(chat ? ' ~' + chat : ''),
