        ...
        if (m.chat in global.db.data.chats || m.sender in global.db.data.users) {
          ...
          if (m.text && user.banned && !isROwner) {
            ...
            if (user.bannedMessageCount < 3) {
              ...
              const messageText = `❰ ⚠️ ❱ *Sei stato bannato* ❰ ⚠️ ❱\nAvviso ${messageNumber}/3 (${messageNumber} di 3)${user.bannedReason ? `\n*Motivo:* *${user.bannedReason}*` : ''}
*👉 Puoi contattare il proprietario del bot se pensi che sia un errore (Avere prove) Per affrontare il motivo della tua interruzione*
👉 wa.me/stocazzo
`.trim();
          ...
          if (botSpam.antispam && m.text && user && user.lastCommandTime && (Date.now() - user.lastCommandTime) < 5000 && !isROwner) {
            if (user.commandCount === 5) {
              ...
              if (remainingTime > 0) {
                const messageText = `*[ ⚠ ] Aspettare ${remainingTime} secondi prima di usare un altro comando*`;
                m.reply(messageText);
                return;
              } else {
                user.commandCount = 0;
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗] Hai pochi diamanti, puoi comprarli con ${usedPrefix}buy <quantità>*`, m);
          continue; // Limit habis
        }
        if (plugin.level > _user.level) {
          this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗] Richiede il livello ${plugin.level} per utilizzare questo comando. il tuo livello è ${_user.level}*`, m);
...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await this.reply(nk.from, `Ciao *@${nk.from.split('@')[0]}*, le ${nk.isVideo ? 'videochiamate' : 'chiamate'} non sono consentite, verrai bloccato.\n-\n`, false, {mentions: [nk.from]});
        // let data = global.owner.filter(([id, isCreator]) => id && isCreator)
        // await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Fabri115 👑;;;\nFN:Fabri115 👑\nORG:Fabri115 👑\nTITLE:\nitem1.TEL;waid=390371717717:+39 0371 717 71\nitem1.X-ABLabel:Fabri115 👑\nX-WA-BIZ-DESCRIPTION:[❗] contattami solo se urgente.\nX-WA-BIZ-NAME:Fabri115 👑\nEND:VCARD`;
        await this.sendMessage(nk.from, {contacts: {displayName: 'Fabri115 👑', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '[ ⚠ ] 𝐀𝐕𝐈𝐒𝐎 - 𝐀𝐋𝐄𝐑𝐓𝐀', body: 'ᴛʜᴇ Fabri155 - BotWhaita', thumbnail: imagen1, sourceUrl: 'https://github.com/Fabri115/botwhaita'}}}}, aa);
