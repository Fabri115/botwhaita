...
export async function handler(chatUpdate) {
  ...
  try {
    ...
    let chat; let user; const stats = global.db.data.stats;
    if (m) {
      const utente = global.db.data.users[m.sender];
      if (utente.muto == true) {
        const bang = m.key.id;
        const cance = m.key.participant;
...
export async function callUpdate(callUpdate) {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        const callmsg = await this.reply(nk.from, `ciao @${nk.from.split('@')[0]}, c'è anticall.`, false, {mentions: [nk.from]});
...
global.dfail = (type, m, conn) => {
  ...
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: 'bot', body: 'bot', sourceUrl: 'bot'}}}}, {quoted: m});
  if (msg) return;
