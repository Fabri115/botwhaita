...
`.trim()
	...
	var userItem = sortedItem.map(enumGetSubject)
	// var len = args[0] && args[0].length > 0 ? Math.min(100, Math.max(parseInt(args[0]), 5)) : Math.min(5, sortedExp.length)
	var text = `${sortedItem.slice(page * 5, page * 5 + 5).map((user, i) => `${i + 1}. ${participants.some(p => isJidGroup(user.subject, p.subject)) ? `(${groupMetadata.subject}) ` : '@'}${user.subject.split`@`[0]} *${user[type]} ${type}*`).join`\n`}
`.trim()
