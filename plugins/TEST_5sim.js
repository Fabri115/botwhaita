...
let bestPrice = 1337; let bestPriceCT = '';
...
const handler = async (m, {conn, args}) => {
  ...
  const product = args[0].toLowerCase(); let txt = '*List Price :*\n';
  for (const country of await getCountryList()) {
    const data = await getProductList(country);
...
async function getCountryList() {
  const res = await axios.get('https://5sim.net/v1/guest/countries', {
    headers: {
...
async function getProductList(country) {
  const res_ = await axios.get('https://5sim.net/v1/guest/products/' + country.toLowerCase() + '/any', {
    headers: {
      'Accept': 'application/json',
      'Authorization': 'Bearer ' + token,
    },
