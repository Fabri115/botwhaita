...
const handler = async (m, {conn, usedPrefix, command, args, isOwner, isAdmin, isROwner, users, chatupdate}) => {
  const locale = 'es';
  ...
  const {exp, diamond, registered, self, level, role} = global.db.data.users[m.sender];
