let handler = async (m, { conn, args, usedPrefix }) => {
    ...
    const filteredParticipants = Object.values(data)
      .filter(({ jid }) => jid.endsWith('@s.whatsapp.net') && jid !== conn.user.jid && data[jid].presence === 'composing')
      .map(({ jid, name }) => ({ jid, name }));
    
    if (filteredParticipants.length === 0) {
        return conn.reply(m.chat, 'Nessun utente online al momento.', m);
    }
